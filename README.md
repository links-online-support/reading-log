...
タイトル・著者・ステータス・評価（星5段階）・カテゴリ・タグ（カンマ区切りで複数登録可）・開始日/完了日・読んだページ数/総ページ数・学びメモを記録できます。カテゴリはフォーム上から追加可能です。ISBNを入力（または対応ブラウザではカメラでバーコードを読み取り）すると、書誌情報APIからタイトル・著者を自動入力できます。既に登録済みの本だった場合は警告を表示しますが、再読の記録もできるよう登録自体はブロックしません。
...
- ISBNから書籍情報を取得した際、既に登録済みの本であれば警告を表示するようにした。ただし「同じ本をもう一度読む」ことは普通にあるため、登録を禁止するのではなく警告に留め、ユーザー自身が判断できるようにしている。判定は取得したISBNの一致に加えてタイトルの一致でも行っており、ISBN機能を使わず手入力で登録していた本にも気づけるようにした
